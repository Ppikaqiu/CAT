...
if [ $NODE == 0 ]; then
	...
	#for part in "test" "dev" "validated"; do
        ...
        local/mozilla_decode_graph_3gram.sh data/local/local_lm data/lang_phn data/lang_phn_test_3gram || exit 1;
	local/mozilla_decode_graph_4gram.sh data/local/local_lm data/lang_phn data/lang_phn_test_4gram || exit 1;
    fi 
...
PARENTDIR='.'
dir="exp/qzy_7_vggblstm_transformerearlystop_specaug"
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
    ...
    for set in test; do
        ...
        mkdir -p $dir/decode_${set}_3gram
        ln -snf $ark_dir $dir/decode_${set}_3gram/logits
        ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_3gram data/$set data/all_ark/$set.ark $dir/decode_${set}_3gram || exit 1

        

	mkdir -p $dir/decode_${set}_4gram
        ln -snf $ark_dir $dir/decode_${set}_4gram/logits
        ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_4gram data/$set data/all_ark/$set.ark $dir/decode_${set}_4gram || exit 1




        grep WER $dir/decode_${set}_3gram*/wer_* | utils/best_wer.sh
        grep WER $dir/decode_${set}_4gram*/wer_* | utils/best_wer.sh

...
stop_stage=100
nj=$(nproc)
data=/root/CAT/egs/Indonesia/data/Indonesia
lang=India
...
nbpe=150
bpemode=char # `char` for char-based system.
...
if [ $NODE == 0 ]; then
    ...
    utils/filter_scp.pl --exclude data/dev_$lang/wav.scp data/train_$lang/wav.scp > data/train_$lang/temp_wav.scp || exit 1;
    utils/filter_scp.pl --exclude data/test_$lang/wav.scp data/train_$lang/temp_wav.scp > data/train_$lang/wav.scp || exit 1;
    utils/fix_data_dir.sh data/train_$lang || exit 1;
...
#     utils/perturb_data_dir_speed.sh 0.9 data/${train_set} data/temp1 || exit 1;
    ...
    utils/combine_data.sh --extra-files utt2uniq data/train_$lang data/temp1 data/temp2 data/temp3 || exit 1;
    ...
    if [ $stage -le 3 ] && [ ${stop_stage} -ge 3 ]; then
      ...
      for x in train dev; do
          python3 ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt data/$x/text_pos \
...
PARENTDIR='.'
dir="exp/cv_de_char" # `exp/cv_de_char` for char-based system

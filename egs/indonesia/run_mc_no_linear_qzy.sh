...
finetune_dir="exp/mc_no_linear_finetune/"
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
    ...
    --mc-conf=mc_conf/mc_no_linear_finetune_Id.json  \
...
if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
    for x in test; do
        ...
        ark_dir=$finetune_dir/decode_${x}_no_linear_few_shot/logits
        mkdir -p $ark_dir
        ark_dir=$(readlink -f $ark_dir)

        python3 ctc-crf/calculate_logits.py                        \
            --mc-conf=mc_conf/mc_no_linear_few_shot_eval_Id.json   \
        ...
        mkdir -p $finetune_dir/decode_${x}_no_linear_3gram_few_shot
        ln -snf $ark_dir  $finetune_dir/decode_${x}_no_linear_3gram_few_shot/logits
        
	ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_3gram data/$x data/all_ark/$x.ark $finetune_dir/decode_${x}_no_linear_3gram_few_shot || exit 1


        mkdir -p $finetune_dir/decode_${x}_no_linear_4gram_few_shot
        ln -snf $ark_dir $finetune_dir/decode_${x}_no_linear_4gram_few_shot/logits
        ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_4gram data/$x data/all_ark/$x.ark $finetune_dir/decode_${x}_no_linear_4gram_few_shot || exit 1


        grep WER $finetune_dir/decode_${x}_no_linear_3gram_few_shot*/wer_* | utils/best_wer.sh
        grep WER $finetune_dir/decode_${x}_no_linear_4gram_few_shot*/wer_* | utils/best_wer.sh
...
if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
    for x in test; do
        scp=data/all_ark/$x.scp
        ark_dir=$finetune_dir/decode_${x}_no_linear_zero_shot/logits
        mkdir -p $ark_dir
        ark_dir=$(readlink -f $ark_dir)

        python3 ctc-crf/calculate_logits.py                         \
            --mc-conf=mc_conf/mc_no_linear_zero_shot_eval_Id.json   \
            --mc-train-pv=npy-lzw/id.npy                            \
            --resume=$dir/bestckpt.pt                               \
            --config=$dir/config.json                               \
            --nj=$nj --input_scp=$scp                               \
        ...
        mkdir -p $finetune_dir/decode_${x}_no_linear_3gram_zero_shot
        ln -snf $ark_dir  $finetune_dir/decode_${x}_no_linear_3gram_zero_shot/logits

        ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_3gram data/$x data/all_ark/$x.ark $finetune_dir/decode_${x}_no_linear_3gram_zero_shot || exit 1


        mkdir -p $finetune_dir/decode_${x}_no_linear_4gram_zero_shot
        ln -snf $ark_dir $finetune_dir/decode_${x}_no_linear_4gram_zero_shot/logits
        ctc-crf/decode.sh  --stage 1 \
            --cmd "$decode_cmd" --nj $nj --acwt 1.0 \
            data/lang_phn_test_4gram data/$x data/all_ark/$x.ark $finetune_dir/decode_${x}_no_linear_4gram_zero_shot || exit 1
